...
"""
Enhanced User model with SSO invitation support - CORRECTED

File: backend/app/models/user.py
Purpose: User model with proper relationships for agents AND invitations
"""
...
class User(Base):
    """
    User model with multi-tenant support and SSO invitation workflow
    Each tenant schema has its own users table
    """
    __tablename__ = "users"
    ...
    # Relationship to agents (existing)
    agents = relationship("AgentConfig", back_populates="creator")
    
    # Relationship to invitation (new - fixed to avoid conflicts)
    # Users who invited this user
    inviter = relationship(
        "User",
        remote_side=[id],
        foreign_keys=[invited_by],
        backref="invited_users"  # This creates the reverse relationship
    )
    
    # Note: We use backref instead of back_populates to avoid circular dependency
    # The backref automatically creates `invited_users` on the parent User
    ...
    @classmethod
    def get_by_invitation_token(cls, db: Session, token: str) -> Optional["User"]:
        """Get user by invitation token"""
        return db.query(cls).filter(
            cls.invitation_token == token,
            cls.invitation_status == 'pending'
        ).first()
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "provisioning_method": self.provisioning_method
        }
