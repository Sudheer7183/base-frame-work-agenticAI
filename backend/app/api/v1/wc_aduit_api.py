...
def _fmt_date(val) -> str:
    """Convert a date/datetime/string to full date display format."""
    if not val:
        return ""
    ...
    try:
        ...
        for fmt in ("%Y-%m-%d", "%m/%d/%Y", "%Y/%m/%d", "%m-%d-%Y"):
            try:
                return _dt.strptime(s, fmt).strftime("%d %b %Y")
            except ValueError:
                pass
...
def _db_case_to_dict(case: "AuditCase", live_hitl_check: bool = True) -> dict:
    ...
    overall_variance = {
        ...
        "annual_estimated_premium":policy.estimated_premium,
    ...
    return {
        ...
        "actual_submissions": case.actual_submissions,
