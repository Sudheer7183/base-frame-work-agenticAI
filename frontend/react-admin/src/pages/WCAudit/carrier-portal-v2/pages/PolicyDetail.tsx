...
//   //   },
//   // ]
//   const [showEmptyFirstMonth, setShowEmptyFirstMonth] = useState(false);
...
//   const firstIsEmpty = (first.earnedPremium ?? 0) === 0;
//   if (firstIsEmpty && !showEmptyFirstMonth) {
//     return chartData.slice(1);   // hide it
//   }
//   return chartData;              // show everything
// }, [chartData, showEmptyFirstMonth]);

//   const firstMonthIsEmpty = chartData?.length > 0 && (chartData[0].earnedPremium ?? 0) === 0;
...
//             <BarChart data={filteredChartData} margin={{ top: 8, right: 20, left: 10, bottom: 0 }} barCategoryGap="40%">
...
//             {/*Payment frequency*/}
//             <div style={{ padding: '12px 14px', borderRadius: 12, background: '#F8FAFD', border: `1px solid ${C.border}` }}>
//               <p style={{ margin: 0, fontSize: 11, color: C.muted, fontWeight: 500 }}>Payroll Submission Frequency</p>
//               <p style={{ margin: '6px 0 0', fontSize: 20, fontWeight: 800, color: C.text }}>
...
//               <p style={{ margin: '6px 0 0', fontSize: 20, fontWeight: 800, color: missingPayrolls > 0 ? '#92400E' : C.text }}>
...
// Fix CustomTooltip — use variance directly instead of recalculating
const CustomTooltip = ({ active, payload, label }: any) => {
  if (!active || !payload?.length) return null

  // Use the variance field passed in chartData — already earned - est
  const variance = payload[0]?.payload?.variance ?? null
  ...
  return (
    <div style={{
      ...
      {variance !== null && (
        <p style={{ marginTop: 4, fontWeight: 700, color: variance >= 0 ? C.red : C.green }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <p style={{ margin: 0, fontSize: 13, color: C.muted }}>
            <strong style={{ color: C.text }}>
              Last Audit Run Date: {policy.completed_at ? formatDate(policy.completed_at) : '—'}
            </strong>
          </p>
        </div>
          ...
          {varAbove && (
            <div style={{
              ...
              <p style={{ margin: 0, fontSize: 13, color: '#991B1B', fontWeight: 500 }}>
                Variance of{' '}
                <strong>{policy.variancePercent.toFixed(1)}%</strong>{' '}
                exceeds the {targetVariance.threshold}% threshold. The Audit run date is 18 May 2026
                {missingPayrolls > 1 &&
                  ` Also flagged for ${missingPayrolls} missing payrolls.`}
              </p>
            </div>
              ...
              <thead>
                <tr>
                  {[
                    ['Class Code', 'left'], ['Description', 'left'],
                    // ['EST Exposure', 'right'], ['Actual Earned Exposure', 'right'],
                    ['Actual Earned Exposure', 'right'],['EST Exposure', 'right'],
                    ['Variance', 'right'], ['Net Rate', 'right'],
                    ['Est earned', 'right'], ['Action', 'left'],
                  ].map(([h, align]) => (
                    <th key={h as string} style={{ ...tableThStyle, textAlign: align as any }}>{h}</th>
